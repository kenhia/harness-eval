...
**Reviewers:** correctness, testing, thermo-nuclear-code-quality, security, api-contract
...
| # | File | Issue | Reviewer | Confidence | Route |
|---|------|-------|----------|------------|-------|
| 5 | `export_helper.rb:12` | Nested format detection makes the surrounding flow harder to scan | thermo-nuclear-code-quality | 0.70 | `advisory -> human` |
...
This fails because: no pipe-delimited tables, no severity-grouped `###` headers, uses box-drawing horizontal rules, no numbered findings, no `## KB Review Results` title, and the verdict is not in a blockquote. Always use the table format from the example above.
...
- **Always include file:line location** for KB review issues
...
- **Schema Drift Check section** -- results from the schema drift check. Omit if the check did not run.
- **Deployment Notes section** -- key checklist items from the deployment verification check. Omit if the check did not run.
