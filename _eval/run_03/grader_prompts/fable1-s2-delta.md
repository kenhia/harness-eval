# Grader prompt — Fable(1): run_03 S2 delta re-grade (cells 03 and 06 only)
...
**Your grader id is `fable1`.** You are running as a CLI session on kai
...
- **Independence**: do not read anything with `sol` in the filename,
  and nothing under `grades/reconcile/`.

## Output

- Overwrite `$EVAL/_eval/run_03/grades/03-fable1.md` and
  `06-fable1.md` with the re-scored sheets (same template).
- Update the two affected rows in
  `$EVAL/_eval/run_03/grades/summary-fable1.md`; leave the other rows
  untouched.
...
- Commit: `git add _eval/run_03/grades && git commit -m
  "grades(fable1): run_03 S2 delta re-grade — cells 03 and 06"`.
