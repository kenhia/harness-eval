# Grader prompt — Fable(1) delta: grade 06-gstack only
...
same scale, then stop. Grader `sol` is doing the same independently; a
delta-consensus session integrates afterwards.

## Environment

Host `kai`, eval repo `/home/ken/src/ai-agents/harness-eval` (`$EVAL`). You
run in a desktop UI: everything happens over ssh, and kai's default shell
is fish — always wrap: `ssh kai bash -lc '...'`.

The new run lives in a STAGING repo (not inside `$EVAL`):
`/home/ken/src/ai-agents/harness-eval-runs/06-gstack` — branch `main`,
tag `pre-run` marks the boundary; agent work = `pre-run..HEAD`.
...
`summary-fable1.md`) to anchor your scale — score 06 as if the same grader
were still holding the pen. Sol's files (`*-sol*`) remain OFF-LIMITS, as
does `grades/reconcile/` and `grades/final.md` (it contains consensus
...
1. Clone: `git clone /home/ken/src/ai-agents/harness-eval-runs/06-gstack /tmp/grade-fable1/06-gstack`
...
2. Objective pass: reuse YOUR sealed fixture
   (`$EVAL/_eval/grades/sealed-fixture-fable1/`) and execute the 12 checks
   from `acceptance.md` → `$EVAL/_eval/grades/06-acceptance-fable1.md`
   (per-check pass/fail + evidence, mapped to the 0–5 correctness score).
3. Rubric pass: score all 7 dimensions per `rubric.md` →
   `$EVAL/_eval/grades/06-fable1.md` (score-sheet template; judge the
   artifact, not the brand; ceremony that didn't change the outcome scores
   LOW on Process).
4. Append the 06 row to `$EVAL/_eval/grades/summary-fable1.md`.
5. Commit from `$EVAL`:
   `git add _eval/grades && git commit -m "grades(fable1): delta pass — 06-gstack"`
   Do NOT push.
