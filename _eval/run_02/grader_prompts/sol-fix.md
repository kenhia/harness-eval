# Grader prompt — GPT Sol: run_02.1 fix-round delta grading
...
5. **Calibration**: your own run_02 sheets (`grades/*-sol.md`,
   `summary-sol.md`). Fable's sheets, `final.md`, `reconcile/` remain
   off-limits.
...
- Never modify run repos; clone to /tmp/grade-sol-fix/ to run tests.
...
1. Per repo, score the five fix-delta dimensions from FIX-ROUND.md
   ...
   `$EVAL/_eval/run_02/grades/NN-fix-sol.md`. Notes:
...
2. `$EVAL/_eval/run_02/grades/summary-fix-sol.md`: one table — rows
   = 6 repos, columns = 5 dimensions + weighted total (/100).
3. Commit: `git add _eval/run_02/grades && git commit -m
   "grades(sol): run_02.1 fix-round delta pass"`. Report the table
