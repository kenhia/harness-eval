# Grader prompt — Fable(1): rubric grading, run_02 field
...
grader (GPT Sol) is doing the same independently; a third session
...
6. **Calibration**: your predecessor sessions' run-1 sheets — `$EVAL/_eval/run_01/grades/*-fable1.md` and `summary-fable1.md`. Your scale must be continuous with theirs. (Sol's sheets, `final.md`, and `reconcile/` remain off-limits, run_01 and run_02 both.)

## Hard rules

- **Independence**: read nothing with `sol` in the filename, nothing under `grades/reconcile/`.
- **Never modify the seven run repos.** Grade against clones:
  `git clone ~/src/ai-agents/harness-eval-runs/run_02/NN-x /tmp/grade-fable1-run02/NN-x`
...
template → `$EVAL/_eval/run_02/grades/NN-fable1.md`. Notes:
...
`$EVAL/_eval/run_02/grades/summary-fable1.md`: one table — rows = repos,
...
From `$EVAL`:
`git add _eval/run_02/grades && git commit -m "grades(fable1): run_02 rubric pass, all seven repos"`

Report your summary table in chat and stop. Do not write deliverables, do
not reconcile, do not peek at Sol's work.
