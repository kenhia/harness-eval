...
`git diff pre-run..HEAD` plus the run log `_eval/run_02/runs/NN-runlog.md`.
...
|---|---|---|---|
| 1 | Functional correctness | 30 | Core 14/14 gets a 4; the fifth point tracks the hard tier (≥9/12 ≈ 5). Core failures cap this dimension ≤3. Results supplied by the acceptance run — do not re-derive |
| 2 | Code quality & simplicity | 20 | Idiomatic Rust, sensible workspace split, no speculative abstraction, no dead code, clean error handling (no unwrap-on-IO in server paths) |
| 3 | Test quality | 15 | Repo's own tests exercise behavior incl. edge cases (dates, dedupe, failure isolation, exit codes) and include a real feedd-vs-feedgen end-to-end test; would catch regressions; not tautological |
| 4 | Documentation | 10 | README alone is enough for a stranger to build, run all three binaries, and use every API endpoint |
...
5. Write to `_eval/run_02/grades/NN-<grader>.md` using the run_01 score
   sheet template.
